...
1.  **Cluster / group characterisation** — the `plot_group_*` family
    ...
    - [`plot_group_heatmap()`](https://satvilab.github.io/UtilsGGSV/reference/plot_group_heatmap.md)
      — ECDF-percentile heat map showing each group’s relative position
      for every variable.
    - [`plot_group_density()`](https://satvilab.github.io/UtilsGGSV/reference/plot_group_density.md)
      — per-variable density plots with per-group overlays (density
      curves and/or median lines).
    - [`plot_group_scatter()`](https://satvilab.github.io/UtilsGGSV/reference/plot_group_scatter.md)
      — biaxial scatter with optional PCA / t-SNE / UMAP projection and
      cluster centroids.
    - [`plot_group_mst()`](https://satvilab.github.io/UtilsGGSV/reference/plot_group_mst.md)
      — minimum-spanning-tree layout coloured by the same ECDF scale as
      the heat map.
2.  **Correlation visualisation** —
    [`ggcorr()`](https://satvilab.github.io/UtilsGGSV/reference/ggcorr.md)
    creates paired scatter plots with Spearman, Pearson, Kendall, or
    concordance correlation coefficients overlaid as a formatted table,
    with support for log / asinh / any `scales` transformation.

Additional helpers round out the toolkit:

- [`axis_limits()`](https://satvilab.github.io/UtilsGGSV/reference/axis_limits.md)
  — force equal axis limits or expand axis coordinates without manually
  computing values.
- [`add_text_column()`](https://satvilab.github.io/UtilsGGSV/reference/add_text_column.md)
  — place a column of text annotations at a consistent relative position
  regardless of the underlying axis transformation.
- [`get_trans()`](https://satvilab.github.io/UtilsGGSV/reference/get_trans.md)
  — retrieve any `scales` transformation by name, including higher-root
  and `asinh` transformations not available in base `scales`.
